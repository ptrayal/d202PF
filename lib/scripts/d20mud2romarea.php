<?php
	...
	$fileNumber = "";								// The name of the source file obtained from the index
	$oString = "";									// output string for file processing
		...
		if (is_file($index)) {
			if (is_readable($index)) {
				...
				while (!feof($indexFile)) {
					...
					$fileNameArray = explode(".", $sourceFileName);
					
					$fileNumber = $fileNameArray[0];
									...
									if (file_exists($outputFileName)) {
										if (is_file($outputFileName)) {
											if (is_writable($outputFileName)) {
												...
												fclose($sourceFile);
												fclose($outputFile);
												rename($fileNumber . ".are", $fullOutputDir . $fileNumber . ".are");
											}
										}
									}
								}
