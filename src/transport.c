...
ACMD(do_starship) {
  ...
  if (!ch->ship1) {
    send_to_char(ch, "You do not own a starship, you will have to use the shuttle command.\r\n");
    return;
  }
  ...
  if (!found) {
    send_to_char(ch, "You are not at a valid spaceport.\r\n");
    return;
  }

  if (ch->ship1->ship_type == 0) {
    send_to_char(ch, "You do not have a star ship in your possession.\r\n");
    return;
  }

  if (get_skill_value(ch, SKILL_PILOT) < atoi(ship_types[ch->ship1->ship_type][3])) {
    send_to_char(ch, "That ship requires a pilot skill of %d to fly, and yours is only %d.\r\n", atoi(ship_types[ch->ship1->ship_type][3]), get_skill_value(ch, SKILL_PILOT));
    return;
  }
  ...
  if (!*argument) {
    found = FALSE;
    i = 0;
    send_to_char(ch, "Available Shuttle Destinations:\r\n");
    send_to_char(ch, "@Y%-30s %4s %10s %10s (%s)\r\n", "Shuttle Destination:", "Cost", "Distance", "Time (sec)", "Area Note");
    int j = 0;
    for (j = 0; j < 80; j++) 
      send_to_char(ch, "~");
    send_to_char(ch, "@n\r\n");
    while (atoi(((CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS) ? shuttle_locales_sw : shuttle_locales_dl)[i][1]) != 0) {
      ...
      if (GET_ROOM_VNUM(IN_ROOM(ch)) != atoi(((CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS) ? shuttle_locales_sw : shuttle_locales_dl)[i][1]) && valid_shuttle_travel(here, i)) {
        found = TRUE;
        send_to_char(ch, "%-30s %4s %10d %10d (%s)\r\n", ((CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS) ? shuttle_locales_sw : shuttle_locales_dl)[i][0],  ((CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS) ? shuttle_locales_sw : shuttle_locales_dl)[i][2], get_distance(ch, i, here), get_travel_time(ch, ch->ship1->ship_speed, i, here), ((CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS) ? shuttle_locales_sw : shuttle_locales_dl)[i][4]);
      }
      i++;
    }

    if (found) {
      send_to_char(ch, "\r\nTo travel in your starship, type @Ystarship <name of destination>@n\r\n");
      return;
    } else {
      send_to_char(ch, "There are no available destinations from this spaceport.\r\n");
    }
    return;
  } else {
    i = 0;
    ...
    while (atoi(((CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS) ? shuttle_locales_sw : shuttle_locales_dl)[i][1]) != 0) {
      if (GET_ROOM_VNUM(IN_ROOM(ch)) != atoi(((CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS) ? shuttle_locales_sw : shuttle_locales_dl)[i][1]) && valid_shuttle_travel(here, i)) {
        if (is_abbrev(argument, ((CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS) ? shuttle_locales_sw : shuttle_locales_dl)[i][0])) {
          ...
          enter_taxi(ch, i, TRAVEL_STARSHIP, here);
        }
      }
      i++;
    }
    if (!found) {
      send_to_char(ch, "There is no starship destination by that name.  Type @Ystarship@n by itself to see a list of destinations.\r\n");
      return;
    }
  }
}
