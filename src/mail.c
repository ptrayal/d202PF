...
SVNHEADER("$Id: mail.c 55 2009-03-20 17:58:56Z pladow $");
...
ACMD(do_mail)
{
  ...
  if (CMD_IS("mail")) {
    postmaster_send_mail(ch, NULL, cmd, argument);
    return;
  } else if (CMD_IS("check")) {
    postmaster_check_mail(ch, NULL, cmd, argument);
    return;
  } else if (CMD_IS("receive")) {
    postmaster_receive_mail(ch, NULL, cmd, argument);
    return;
  } else
    return;
			  ...
			  int cmd, char *arg)
{
  if (has_mail(GET_IDNUM(ch)))
    ...
    act("Sorry, you don't have any mail waiting.", FALSE, mailman, 0, ch, TO_VICT);
}
			  ...
			  int cmd, char *arg)
{
  ...
  while (has_mail(GET_IDNUM(ch))) {
    ...
    act("You receive a piece of mail.", FALSE, mailman, 0, ch, TO_VICT);
    act("$n receives a piece of mail.", FALSE, ch, 0, mailman, TO_ROOM);
  }
}
