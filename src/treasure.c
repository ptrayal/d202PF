...
void award_magic_armor(struct char_data *ch, int grade, int moblevel)
{
    ...
    int size = 0;
    int ac_bonus = 0;
    int rare = 0, raregrade = 0;
    struct obj_data *obj = NULL;

    /* -------------------------
       Assign Armor Power Rating
       ------------------------- */
    ...
    if (!shield) {
        ...
        if ((type == 100 && roll <= 10) || type == SPEC_ARMOR_TYPE_CLOTHING) {
            ...
            if ((type == 100 && roll <= 8) || type == SPEC_ARMOR_TYPE_PADDED) {
                ...
                material = MATERIAL_COTTON;
            } else if ((type == 100 && roll <= 16) || type == SPEC_ARMOR_TYPE_LEATHER) {
                type = SPEC_ARMOR_TYPE_LEATHER;
                material = MATERIAL_LEATHER;
            } else if ((type == 100 && roll <= 24) || type == SPEC_ARMOR_TYPE_HIDE) {
                type = SPEC_ARMOR_TYPE_HIDE;
                material = MATERIAL_LEATHER;
            } else if ((type == 100 && roll <= 32) || type == SPEC_ARMOR_TYPE_STUDDED_LEATHER) {
                type = SPEC_ARMOR_TYPE_STUDDED_LEATHER;
                material = MATERIAL_LEATHER;
            } else if ((type == 100 && roll <= 40) || type == SPEC_ARMOR_TYPE_LIGHT_CHAIN)
    ...
    /* -------------------------
       Base Description Creation
       ------------------------- */
    if (shield) {
        if (raregrade == 0)
            snprintf(desc, sizeof(desc), "a");
        else if (raregrade == 1)
            snprintf(desc, sizeof(desc), "@G[Rare]@n a");
        else if (raregrade == 2)
            snprintf(desc, sizeof(desc), "@Y[Legendary]@n a");
        else
            snprintf(desc, sizeof(desc), "@M[Mythical]@n a");
    } else {
        if (raregrade == 0)
            snprintf(desc, sizeof(desc), "a suit of");
        else if (raregrade == 1)
            snprintf(desc, sizeof(desc), "@G[Rare]@n a suit of");
        else if (raregrade == 2)
            snprintf(desc, sizeof(desc), "@Y[Legendary]@n a suit of");
        else
            snprintf(desc, sizeof(desc), "@M[Mythical]@n a suit of");
    }
    ...
    if (desc1 == 3) {
        ...
        snprintf(temp, sizeof(temp), "%s %s", desc, armor_special_descs[armor_special_desc_roll]);
        strlcpy(desc, temp, sizeof(desc));
    }
    ...
    if (desc2 == 3 || desc2 == 4) {
        ...
        snprintf(temp, sizeof(temp), "%s%s %s", desc, desc1 == 3 ? "," : "", colors[color1]);
        strlcpy(desc, temp, sizeof(desc));
    } else if (desc2 == 5) {
        i = 0;
        while (*(colors + i++)) {}
        ...
        snprintf(temp, sizeof(temp), "%s%s %s and %s", desc, desc1 == 3 ? "," : "", colors[color1], colors[color2]);
        strlcpy(desc, temp, sizeof(desc));
    }

    /* Material Type */
    snprintf(temp, sizeof(temp), "%s %s", desc, material_names[material]);
    strlcpy(desc, temp, sizeof(desc));

    /* Armor Type */
    if (type == SPEC_ARMOR_TYPE_CLOTHING && dice(1, 2) == 1) {
        snprintf(temp, sizeof(temp), "%s robes", desc);
        strlcpy(desc, temp, sizeof(desc));
        robes = TRUE;
    } else {
        snprintf(temp, sizeof(temp), "%s %s", desc, armor_list[type].name);
        strlcpy(desc, temp, sizeof(desc));
    }
    ...
    if (desc3 >= 7) {
        ...
        snprintf(temp, sizeof(temp), "%s with %s %s crest", desc, AN(armor_crests[crest]), armor_crests[crest]);
        strlcpy(desc, temp, sizeof(desc));
    } else if (desc3 >= 5) {
        i = 0;
        while (*(armor_crests + i++)) {}
        size = i;
        crest = MAX(0, dice(1, (int)size) - 2);
        snprintf(temp, sizeof(temp), "%s covered in symbols of %s %s", desc, AN(armor_crests[crest]), armor_crests[crest]);
        strlcpy(desc, temp, sizeof(desc));
    }

    /* Shield article fix */
    if (shield) {
        snprintf(newDesc, sizeof(newDesc), "Null");
        for (i = 2; desc[i]; i++)
            newDesc[i - 2] = desc[i];
        newDesc[i - 2] = '\0';
        snprintf(temp, sizeof(temp), "%s %s", AN(newDesc), newDesc);
        strlcpy(desc, temp, sizeof(desc));
    }
    ...
    if (ac_bonus) {
        obj->affected[0].location = shield ? APPLY_AC_SHIELD : APPLY_AC_ARMOR;
        obj->affected[0].modifier = ac_bonus;
    }

    snprintf(keywords, sizeof(keywords), "%s %s %s %s %s %s",
             material_names[material],
             color1 ? colors[color1] : "",
             color2 ? colors[color2] : "",
             robes ? "robes" : armor_list[type].name,
             shield ? "shield" : "suit",
             crest ? armor_crests[crest] : "");

    obj->name = strdup(keywords);
    obj->short_description = strdup(desc);

    snprintf(temp, sizeof(temp), "%s is lying here.", desc);
    obj->description = strdup(temp);
    ...
    if (!(IS_NPC(ch) && IS_MOB(ch) && GET_MOB_SPEC(ch) == shop_keeper)) {
        send_to_char(ch, "@YYou have found %s in a nearby lair!@n\r\n", obj->short_description);
        snprintf(buf, sizeof(buf), "@Y$n has found %s in a nearby lair!@n", obj->short_description);
        act(buf, FALSE, ch, 0, ch, TO_NOTVICT);
    }
}
