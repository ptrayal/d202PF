...
case CLASS_SACRED_FIST:
    ...
    if (!GET_SKILL(ch, SKILL_TUMBLE) < 5) {
    	if (show_text) send_to_char(ch, "You must have at least 5 ranks in the tumble skill to become a sacred fist.\r\n");
    } else {
      c2 = TRUE;
    }
...
case CLASS_DEATH_MASTER:
    if (GET_LEVEL(ch) < 1)
      return (0);
    if (GET_SKILL(ch, SKILL_LORE) < 8) {
    	if (show_text) send_to_char(ch, "You must have a lore skill of at least 8 to become a pale master.\r\n");
    } else {
      c1 = TRUE;
    }
  ...
  case CLASS_ARCANE_TRICKSTER:
    ...
    if (GET_SKILL(ch, SKILL_LORE) < 4) {
    	if (show_text) send_to_char(ch, "You must have a lore skill of at least 4 to become an arcane trickster.\r\n");
    } else {
      c2 = TRUE;
    }
    ...
    if (GET_SKILL(ch, SKILL_DECIPHER_SCRIPT) < 7) {
    	if (show_text) send_to_char(ch, "You must have a decipher script skill of at least 7 to become an arcane trickster.\r\n");
    } else {
      c3 = TRUE;
    }
  ...
  case CLASS_ASSASSIN:
    ...
    if (GET_SKILL(ch, SKILL_HIDE) < 8) {
      if (show_text) send_to_char(ch, "You need a hide skill of at least 8 to become an assassin.\r\n");
    } else {
      c2 = TRUE;
    }
    if (GET_SKILL(ch, SKILL_MOVE_SILENTLY) < 8) {
      if (show_text) send_to_char(ch, "You need a move silently skill of at least 8 to become an assassin.\r\n");
    } else {
      c3 = TRUE;
    }
    if (c1 && c2 && c3)
  ...
  case CLASS_DRAGON_DISCIPLE:
    ...
    if (GET_SKILL(ch, SKILL_LORE) >= 8)
      ...
      if (show_text) send_to_char(ch, "You need a lore skill of at least 8 to qualify for a dragon disciple.\r\n");
  ...
  case CLASS_MYSTIC_THEURGE:
    if (GET_LEVEL(ch) < 1)
      return (0);
    if (GET_SKILL(ch, SKILL_LORE) >= 6)
      c1 = TRUE;
    else
      if (show_text) send_to_char(ch, "You must have a minimum lore skill of 6 to become a mystic theurge.\r\n");
    GET_MEM_TYPE(ch) = MEM_TYPE_CLERIC;
...
//  struct obj_data * objNew, * objCont;	
  ...
  case RACE_BAAZ_DRACONIAN:
    GET_MAX_HIT(ch) += 24;
    GET_ACCURACY_BASE(ch) = 2;
    GET_SAVE_BASE(ch, SAVING_FORTITUDE) += 3;
    GET_SAVE_BASE(ch, SAVING_REFLEX)  += 3;
    GET_SAVE_BASE(ch, SAVING_WILL)  += 3;
    GET_PRACTICES(ch, GET_CLASS(ch)) = 6;
    GET_PRACTICES(ch, GET_CLASS(ch)) += ability_mod_value(ch->real_abils.intel);
    GET_PRACTICES(ch, GET_CLASS(ch)) = MAX(1, GET_PRACTICES(ch, GET_CLASS(ch)));   
    GET_PRACTICES(ch, GET_CLASS(ch)) *= 5;
    SET_FEAT(ch, FEAT_SIMPLE_WEAPON_PROFICIENCY, 1);
    SET_FEAT(ch, FEAT_ARMOR_PROFICIENCY_LIGHT, 1);
    SET_FEAT(ch, FEAT_ARMOR_PROFICIENCY_SHIELD, 1);
    GET_FEAT_POINTS(ch) = 1;
    break;
  ...
  case RACE_KAPAK_DRACONIAN:
    ...
    SET_FEAT(ch, FEAT_SIMPLE_WEAPON_PROFICIENCY, 1);
    SET_FEAT(ch, FEAT_ARMOR_PROFICIENCY_LIGHT, 1);
    SET_FEAT(ch, FEAT_SNEAK_ATTACK, 1);
    GET_FEAT_POINTS(ch) = 1;
    break;
...
int *free_start_feats[] = {
 ...
 /* CLASS_KOCR  */   free_start_feats_knight_of_crown,
 /* CLASS_KOSW  */   free_start_feats_knight_of_sword,    
 /* CLASS_KORO  */   free_start_feats_knight_of_rose,
 /* CLASS_KOLI  */   free_start_feats_none,
 /* CLASS_KOSK  */   free_start_feats_none,
 /* CLASS_KOTH  */   free_start_feats_none,    
 /* CLASS_WHS   */   free_start_feats_wizard_of_high_sorcery,
 /*CLASS_DUELIST*/   free_start_feats_duelist,
 /* CLASS_GLAD  */   free_start_feats_gladiator,
 /* CLASS_MYST  */   free_start_feats_mystic,    
 /* CLASS_SORC  */   free_start_feats_sorcerer,
 /* CLASS_NOBL  */   free_start_feats_noble,
 /* CLASS_DDEF  */   free_start_feats_none,
 /* CLASS_WMAS  */   free_start_feats_none,       
 /* CLASS_DDIS  */   free_start_feats_none,       
 /* CLASS_AARC  */   free_start_feats_arcane_archer,       
 /* CLASS_IBLA  */   free_start_feats_none,       
 /* CLASS_NPC   */   free_start_feats_none,       
 /* CLASS_ASSN  */   free_start_feats_none,       
 /* CLASS_BLCK  */   free_start_feats_none,       
 /* CLASS_ARTSN */   free_start_feats_artisan,       
 /* CLASS_ATRK  */   free_start_feats_none,       
 /* CLASS_FSOL  */   free_start_feats_favored_soul,   
 /* CLASS_ELDKN */   free_start_feats_none,       
 /* CLASS_DEATM */   free_start_feats_none,
...
int level_feats[][6] = {
	...
	{CLASS_BARBARIAN, RACE_UNDEFINED,      FALSE,  1, FEAT_RAGE, TRUE},
...
//	{CLASS_UNDEFINED, RACE_SILVANESTI_ELF, FALSE,  1, FEAT_LOW_LIGHT_VISION, TRUE},
	{CLASS_UNDEFINED, RACE_QUALINESTI_ELF, FALSE,  1, FEAT_LOW_LIGHT_VISION, TRUE},
//	{CLASS_UNDEFINED, RACE_KAGONESTI_ELF,  FALSE,  1, FEAT_LOW_LIGHT_VISION, TRUE},
//	{CLASS_UNDEFINED, RACE_DARGONESTI_ELF, FALSE,  1, FEAT_LOW_LIGHT_VISION, TRUE},
//	{CLASS_UNDEFINED, RACE_DIMERNESTI_ELF, FALSE,  1, FEAT_LOW_LIGHT_VISION, TRUE},
//	{CLASS_UNDEFINED, RACE_HALF_ELF,       FALSE,  1, FEAT_LOW_LIGHT_VISION, TRUE},
//	{CLASS_UNDEFINED, RACE_SILVANESTI_ELF, FALSE,  1, FEAT_WEAPON_PROFICIENCY_ELF, TRUE},
	{CLASS_UNDEFINED, RACE_QUALINESTI_ELF, FALSE,  1, FEAT_WEAPON_PROFICIENCY_ELF, TRUE},
//	{CLASS_UNDEFINED, RACE_KAGONESTI_ELF,  FALSE,  1, FEAT_WEAPON_PROFICIENCY_ELF, TRUE},
//	{CLASS_UNDEFINED, RACE_DARGONESTI_ELF, FALSE,  1, FEAT_WEAPON_PROFICIENCY_ELF, TRUE},
//	{CLASS_UNDEFINED, RACE_DIMERNESTI_ELF, FALSE,  1,FEAT_WEAPON_PROFICIENCY_ELF, TRUE},
//	{CLASS_UNDEFINED, RACE_HALF_ELF,       FALSE,  1, FEAT_WEAPON_PROFICIENCY_ELF, TRUE},
#endif
...
void init_skill_classes(void)
{
  ...
  for (i = 0; i < NUM_CLASSES; i++) {
    ...
    skill_class(SKILL_USE_MAGIC_DEVICE, i, SKLEARN_CANT);	
  ...
  skill_class(SKILL_HIDE, CLASS_ARCANE_ARCHER, SKLEARN_CLASS);
  skill_class(SKILL_SPOT, CLASS_ARCANE_ARCHER, SKLEARN_CLASS);
  skill_class(SKILL_RIDE, CLASS_ARCANE_ARCHER, SKLEARN_CLASS);
  skill_class(SKILL_ATHLETICS, CLASS_ARCANE_ARCHER, SKLEARN_CLASS);
  skill_class(SKILL_SURVIVAL, CLASS_ARCANE_ARCHER, SKLEARN_CLASS);


  /* DEATH MASTER */
  skill_class(SKILL_CONCENTRATION, CLASS_DEATH_MASTER, SKLEARN_CLASS);
  skill_class(SKILL_DIPLOMACY, CLASS_DEATH_MASTER, SKLEARN_CLASS);
  skill_class(SKILL_HEAL, CLASS_DEATH_MASTER, SKLEARN_CLASS);
  skill_class(SKILL_HIDE, CLASS_DEATH_MASTER, SKLEARN_CLASS);
  skill_class(SKILL_LISTEN, CLASS_DEATH_MASTER, SKLEARN_CLASS);
  skill_class(SKILL_LORE, CLASS_DEATH_MASTER, SKLEARN_CLASS);
  skill_class(SKILL_MOVE_SILENTLY, CLASS_DEATH_MASTER, SKLEARN_CLASS);
  skill_class(SKILL_SPELLCRAFT, CLASS_DEATH_MASTER, SKLEARN_CLASS);

  /* ELDRITCH KNIGHT */
  skill_class(SKILL_CONCENTRATION, CLASS_ELDRITCH_KNIGHT, SKLEARN_CLASS);
  skill_class(SKILL_DECIPHER_SCRIPT, CLASS_ELDRITCH_KNIGHT, SKLEARN_CLASS);
  skill_class(SKILL_ATHLETICS, CLASS_ELDRITCH_KNIGHT, SKLEARN_CLASS);
  ...
  skill_class(SKILL_CONCENTRATION, CLASS_FAVORED_SOUL, SKLEARN_CLASS);
  ...
  skill_class(SKILL_SPELLCRAFT, CLASS_ARTISAN, SKLEARN_CLASS);
  skill_class(SKILL_DIPLOMACY, CLASS_ARTISAN, SKLEARN_CLASS);
  skill_class(SKILL_BLUFF, CLASS_ARTISAN, SKLEARN_CLASS);
  skill_class(SKILL_BLUFF, CLASS_ARTISAN, SKLEARN_CLASS);
  skill_class(SKILL_HEAL, CLASS_ARTISAN, SKLEARN_CLASS);
  skill_class(SKILL_USE_MAGIC_DEVICE, CLASS_ARTISAN, SKLEARN_CLASS);
  skill_class(SKILL_SPOT, CLASS_ARTISAN, SKLEARN_CLASS);
  skill_class(SKILL_GATHER_INFORMATION, CLASS_ARTISAN, SKLEARN_CLASS);
  skill_class(SKILL_LISTEN, CLASS_ARTISAN, SKLEARN_CLASS);
  skill_class(SKILL_HANDLE_ANIMAL, CLASS_ARTISAN, SKLEARN_CLASS);
  ...
  skill_class(SKILL_CONCENTRATION, CLASS_SORCERER, SKLEARN_CLASS);
  ...
  skill_class(SKILL_ATHLETICS, CLASS_ROGUE, SKLEARN_CLASS);
  /* FIGHTERS */
  ...
  skill_class(SKILL_ATHLETICS, CLASS_MONK, SKLEARN_CLASS);
  // Sacred Fists
  skill_class(SKILL_TUMBLE, CLASS_SACRED_FIST, SKLEARN_CLASS);
  skill_class(SKILL_BALANCE, CLASS_SACRED_FIST, SKLEARN_CLASS);
  skill_class(SKILL_CONCENTRATION, CLASS_SACRED_FIST, SKLEARN_CLASS);
  ...
  skill_class(SKILL_HIDE, CLASS_SACRED_FIST, SKLEARN_CLASS);
  skill_class(SKILL_LISTEN, CLASS_SACRED_FIST, SKLEARN_CLASS);
  skill_class(SKILL_MOVE_SILENTLY, CLASS_SACRED_FIST, SKLEARN_CLASS);
  skill_class(SKILL_PERFORM, CLASS_SACRED_FIST, SKLEARN_CLASS);
  skill_class(SKILL_ATHLETICS, CLASS_SACRED_FIST, SKLEARN_CLASS);
  ...
  /* BARBARIANS */
  skill_class(SKILL_ATHLETICS, CLASS_BARBARIAN, SKLEARN_CLASS);
  ...
  /* RANGERS */
  skill_class(SKILL_ATHLETICS, CLASS_RANGER, SKLEARN_CLASS);
  skill_class(SKILL_CONCENTRATION, CLASS_RANGER, SKLEARN_CLASS);
  ...
  // Druids
  skill_class(SKILL_CONCENTRATION, CLASS_DRUID, SKLEARN_CLASS);            
  ...
  skill_class(SKILL_ATHLETICS, CLASS_DRUID, SKLEARN_CLASS);            
  // Bards
  ...
  skill_class(SKILL_ATHLETICS, CLASS_BARD, SKLEARN_CLASS);
  skill_class(SKILL_CONCENTRATION, CLASS_BARD, SKLEARN_CLASS);
  ...
  skill_class(SKILL_GATHER_INFORMATION, CLASS_BARD, SKLEARN_CLASS);
  ...
  // Dragon Disciples
  skill_class(SKILL_CONCENTRATION, CLASS_DRAGON_DISCIPLE, SKLEARN_CLASS);
  ...
  skill_class(SKILL_GATHER_INFORMATION, CLASS_DRAGON_DISCIPLE, SKLEARN_CLASS);
  skill_class(SKILL_LISTEN, CLASS_DRAGON_DISCIPLE, SKLEARN_CLASS);
  skill_class(SKILL_SEARCH, CLASS_DRAGON_DISCIPLE, SKLEARN_CLASS);
  skill_class(SKILL_SPELLCRAFT, CLASS_DRAGON_DISCIPLE, SKLEARN_CLASS);
  skill_class(SKILL_SPOT, CLASS_DRAGON_DISCIPLE, SKLEARN_CLASS);

 // Assassins
  skill_class(SKILL_BALANCE, CLASS_ASSASSIN, SKLEARN_CLASS);
  skill_class(SKILL_BLUFF, CLASS_ASSASSIN, SKLEARN_CLASS);
  skill_class(SKILL_ATHLETICS, CLASS_ASSASSIN, SKLEARN_CLASS);
  skill_class(SKILL_BALANCE, CLASS_ASSASSIN, SKLEARN_CLASS);
  skill_class(SKILL_DECIPHER_SCRIPT, CLASS_ASSASSIN, SKLEARN_CLASS);
  ...
  skill_class(SKILL_FORGERY, CLASS_ASSASSIN, SKLEARN_CLASS);
  skill_class(SKILL_GATHER_INFORMATION, CLASS_ASSASSIN, SKLEARN_CLASS);
  skill_class(SKILL_HIDE, CLASS_ASSASSIN, SKLEARN_CLASS);
  skill_class(SKILL_INTIMIDATE, CLASS_ASSASSIN, SKLEARN_CLASS);
  skill_class(SKILL_LISTEN, CLASS_ASSASSIN, SKLEARN_CLASS);
  skill_class(SKILL_MOVE_SILENTLY, CLASS_ASSASSIN, SKLEARN_CLASS);
  skill_class(SKILL_OPEN_LOCK, CLASS_ASSASSIN, SKLEARN_CLASS);
  skill_class(SKILL_SEARCH, CLASS_ASSASSIN, SKLEARN_CLASS);
  ...
  skill_class(SKILL_SPOT, CLASS_ASSASSIN, SKLEARN_CLASS);
  skill_class(SKILL_TUMBLE, CLASS_ASSASSIN, SKLEARN_CLASS);
  skill_class(SKILL_USE_MAGIC_DEVICE, CLASS_ASSASSIN, SKLEARN_CLASS);
  skill_class(SKILL_USE_ROPE, CLASS_ASSASSIN, SKLEARN_CLASS);

  // Arcane Trickster
  skill_class(SKILL_DIPLOMACY, CLASS_ARCANE_TRICKSTER, SKLEARN_CLASS);
  skill_class(SKILL_BALANCE, CLASS_ARCANE_TRICKSTER, SKLEARN_CLASS);
  skill_class(SKILL_BLUFF, CLASS_ARCANE_TRICKSTER, SKLEARN_CLASS);
  skill_class(SKILL_ATHLETICS, CLASS_ARCANE_TRICKSTER, SKLEARN_CLASS);
  skill_class(SKILL_CONCENTRATION, CLASS_ARCANE_TRICKSTER, SKLEARN_CLASS);
  skill_class(SKILL_DECIPHER_SCRIPT, CLASS_ARCANE_TRICKSTER, SKLEARN_CLASS);
  ...
  skill_class(SKILL_GATHER_INFORMATION, CLASS_ARCANE_TRICKSTER, SKLEARN_CLASS);
  skill_class(SKILL_HIDE, CLASS_ARCANE_TRICKSTER, SKLEARN_CLASS);
  skill_class(SKILL_LISTEN, CLASS_ARCANE_TRICKSTER, SKLEARN_CLASS);
  skill_class(SKILL_MOVE_SILENTLY, CLASS_ARCANE_TRICKSTER, SKLEARN_CLASS);
  skill_class(SKILL_OPEN_LOCK, CLASS_ARCANE_TRICKSTER, SKLEARN_CLASS);
  skill_class(SKILL_SEARCH, CLASS_ARCANE_TRICKSTER, SKLEARN_CLASS);
  ...
  skill_class(SKILL_SPOT, CLASS_ARCANE_TRICKSTER, SKLEARN_CLASS);
  skill_class(SKILL_TUMBLE, CLASS_ARCANE_TRICKSTER, SKLEARN_CLASS);
  skill_class(SKILL_USE_ROPE, CLASS_ARCANE_TRICKSTER, SKLEARN_CLASS);
  ...
  skill_class(SKILL_DECIPHER_SCRIPT, CLASS_PURPLE_DRAGON_KNIGHT, SKLEARN_CANT);
  ...
  skill_class(SKILL_DECIPHER_SCRIPT, CLASS_PURPLE_DRAGON_TEMPLAR, SKLEARN_CANT);
  ...
  skill_class(SKILL_CONCENTRATION, CLASS_PURPLE_DRAGON_TEMPLAR, SKLEARN_CLASS);
  //skill_class(SKILL_DIPLOMACY, CLASS_PURPLE_DRAGON_TEMPLAR, SKLEARN_CLASS);
  //skill_class(SKILL_DIPLOMACY, CLASS_PURPLE_DRAGON_TEMPLAR, SKLEARN_CLASS);   
  /* ROSE KNIGHTS */
  skill_class(SKILL_DIPLOMACY, CLASS_PURPLE_DRAGON_LORD, SKLEARN_CLASS);
  skill_class(SKILL_DECIPHER_SCRIPT, CLASS_PURPLE_DRAGON_LORD, SKLEARN_CANT);
  ...
  //skill_class(SKILL_DIPLOMACY, CLASS_PURPLE_DRAGON_LORD, SKLEARN_CLASS);       
  /* DUELISTS */
  skill_class(SKILL_DECIPHER_SCRIPT, CLASS_DUELIST, SKLEARN_CANT);
  skill_class(SKILL_USE_MAGIC_DEVICE, CLASS_DUELIST, SKLEARN_CANT);  
  skill_class(SKILL_BALANCE, CLASS_DUELIST, SKLEARN_CLASS);
  skill_class(SKILL_BLUFF, CLASS_DUELIST, SKLEARN_CLASS);
  skill_class(SKILL_ESCAPE_ARTIST, CLASS_DUELIST, SKLEARN_CLASS);
  skill_class(SKILL_LISTEN, CLASS_DUELIST, SKLEARN_CLASS);
  skill_class(SKILL_PERFORM, CLASS_DUELIST, SKLEARN_CLASS);
  skill_class(SKILL_SENSE_MOTIVE, CLASS_DUELIST, SKLEARN_CLASS);
  skill_class(SKILL_SPOT, CLASS_DUELIST, SKLEARN_CLASS);
  skill_class(SKILL_TUMBLE, CLASS_DUELIST, SKLEARN_CLASS);  
  //skill_class(SKILL_BALANCE, CLASS_DUELIST, SKLEARN_CLASS); 
  // LILY_KNIGHTS
  skill_class(SKILL_DECIPHER_SCRIPT, CLASS_ZHENTARIM_KNIGHT, SKLEARN_CANT);
  ...
  //SKULL_KNIGHTS
  skill_class(SKILL_DECIPHER_SCRIPT, CLASS_ZHENTARIM_PRIEST, SKLEARN_CANT);
  skill_class(SKILL_USE_MAGIC_DEVICE, CLASS_ZHENTARIM_PRIEST, SKLEARN_CANT);    
  skill_class(SKILL_CONCENTRATION, CLASS_ZHENTARIM_PRIEST, SKLEARN_CLASS);     
  ...
  //THORN_KNIGHTS               
  skill_class(SKILL_DECIPHER_SCRIPT, CLASS_ZHENTARIM_WIZARD, SKLEARN_CANT);
  skill_class(SKILL_USE_MAGIC_DEVICE, CLASS_ZHENTARIM_WIZARD, SKLEARN_CANT);    
  skill_class(SKILL_CONCENTRATION, CLASS_ZHENTARIM_WIZARD, SKLEARN_CLASS); 
  ...
  skill_class(SKILL_DECIPHER_SCRIPT, CLASS_KNIGHT_OF_THE_CROWN, SKLEARN_CANT);
  ...
  skill_class(SKILL_DECIPHER_SCRIPT, CLASS_KNIGHT_OF_THE_SWORD, SKLEARN_CANT);
  ...
  skill_class(SKILL_CONCENTRATION, CLASS_KNIGHT_OF_THE_SWORD, SKLEARN_CLASS);
  /* ROSE KNIGHTS */
  skill_class(SKILL_DIPLOMACY, CLASS_KNIGHT_OF_THE_ROSE, SKLEARN_CLASS);
  skill_class(SKILL_DECIPHER_SCRIPT, CLASS_KNIGHT_OF_THE_ROSE, SKLEARN_CANT);
  ...
  skill_class(SKILL_BLUFF, CLASS_DUELIST, SKLEARN_CLASS);
  skill_class(SKILL_ESCAPE_ARTIST, CLASS_DUELIST, SKLEARN_CLASS);
  skill_class(SKILL_ATHLETICS, CLASS_DUELIST, SKLEARN_CLASS);
  skill_class(SKILL_LISTEN, CLASS_DUELIST, SKLEARN_CLASS);
  skill_class(SKILL_PERFORM, CLASS_DUELIST, SKLEARN_CLASS);
  skill_class(SKILL_SENSE_MOTIVE, CLASS_DUELIST, SKLEARN_CLASS);
  skill_class(SKILL_SPOT, CLASS_DUELIST, SKLEARN_CLASS);
  skill_class(SKILL_TUMBLE, CLASS_DUELIST, SKLEARN_CLASS);  
  // LILY_KNIGHTS
  skill_class(SKILL_DECIPHER_SCRIPT, CLASS_KNIGHT_OF_THE_LILY, SKLEARN_CANT);
  skill_class(SKILL_USE_MAGIC_DEVICE, CLASS_KNIGHT_OF_THE_LILY, SKLEARN_CANT);    
  skill_class(SKILL_ATHLETICS, CLASS_KNIGHT_OF_THE_LILY, SKLEARN_CLASS);   
  ...
  //SKULL_KNIGHTS
  skill_class(SKILL_DECIPHER_SCRIPT, CLASS_KNIGHT_OF_THE_SKULL, SKLEARN_CANT);
  skill_class(SKILL_USE_MAGIC_DEVICE, CLASS_KNIGHT_OF_THE_SKULL, SKLEARN_CANT);    
  skill_class(SKILL_CONCENTRATION, CLASS_KNIGHT_OF_THE_SKULL, SKLEARN_CLASS);     
  ...
  //THORN_KNIGHTS               
  skill_class(SKILL_DECIPHER_SCRIPT, CLASS_KNIGHT_OF_THE_THORN, SKLEARN_CANT);
  skill_class(SKILL_USE_MAGIC_DEVICE, CLASS_KNIGHT_OF_THE_THORN, SKLEARN_CANT);    
  skill_class(SKILL_CONCENTRATION, CLASS_KNIGHT_OF_THE_THORN, SKLEARN_CLASS); 
  ...
  skill_class(SKILL_CONCENTRATION, CLASS_MYSTIC_THEURGE, SKLEARN_CLASS);   
  ...
  skill_class(SKILL_LISTEN, CLASS_DWARVEN_DEFENDER, SKLEARN_CLASS);   
  skill_class(SKILL_SENSE_MOTIVE, CLASS_DWARVEN_DEFENDER, SKLEARN_CLASS);   
  skill_class(SKILL_SPOT, CLASS_DWARVEN_DEFENDER, SKLEARN_CLASS);   

  skill_class(SKILL_DECIPHER_SCRIPT, CLASS_WEAPON_MASTER, SKLEARN_CANT);
  skill_class(SKILL_SURVIVAL, CLASS_WEAPON_MASTER, SKLEARN_CANT);
  skill_class(SKILL_RIDE, CLASS_WEAPON_MASTER, SKLEARN_CLASS);
  skill_class(SKILL_ATHLETICS, CLASS_WEAPON_MASTER, SKLEARN_CLASS);
  skill_class(SKILL_USE_ROPE, CLASS_WEAPON_MASTER, SKLEARN_CLASS);
...
/* KNIGHT 		*/ class_feats_champion,
/* TEMPLAR      	*/ class_feats_champion,
/* CHAMPION		*/ class_feats_champion,
/* DRAGON KNIGHT	*/ no_class_feats,
/* DRAGON PRIEST	*/ no_class_feats,
/* DRAGON WIZARD	*/ no_class_feats,
/* HIGH WIZARD		*/ no_class_feats,
/* DUELIST		*/ no_class_feats,
/* GLADIATOR		*/ no_class_feats,
/* MYSTIC THEURGE	*/ no_class_feats,
/* SORCERER		*/ no_class_feats,
/* CLASSLESS		*/ no_class_feats,
/* DWARVEN DEFENDER	*/ class_feats_defender,
/* WEAPON MASTER	*/ class_feats_champion,
/* DRAGON DISCIPLE	*/ class_feats_dragon_disciple,
/* ARCANE ARCHER	*/ class_feats_arcane_archer,
/* INVISIBLE BLADE	*/ no_class_feats,
/* NPC		        */ no_class_feats,
/* ASSASSIN             */ class_feats_assassin,
/* BLACKGUARD           */ no_class_feats,
/* ARTISAN              */ no_class_feats,
/* ARCANE TRICKSTER     */ class_feats_arcane_trickster,
/* FAVORED SOUL         */ class_feats_favored_soul,
/* ELDRITCH KNIGHT      */ class_feats_eldritch_knight,
/* PALE MASTER          */ class_feats_wizard,
/* SACRED FIST          */ class_feats_monk,
/* DRAGON RIDER         */ class_feats_fighter,
/* EXPANSION            */ no_class_feats,
/* EXPANSION            */ no_class_feats,
/* EXPANSION            */ no_class_feats,
/* EXPANSION            */ no_class_feats,
/* EXPANSION            */ no_class_feats,
