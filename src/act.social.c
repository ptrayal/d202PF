...
ACMD(do_action)
{
    ...
    {
        send_to_char(ch, "You cannot do socials when muted.\r\n");
        return;
    }
...
ACMD(do_insult)
{
    ...
    if (*arg)
    {
        ...
        else
        {
            if (victim != ch)
            {
                send_to_char(ch, "You insult %s.\r\n", has_intro(ch, victim) ? GET_NAME(victim) : GET_SHORT_DESC(victim));
...
void boot_social_messages(void)
{
    FILE *fl;
    char *trash = NULL;
    char next_soc[MAX_STRING_LENGTH] = {'\0'};
    char sorted[MAX_INPUT_LENGTH] = {'\0'};
    int itrash = 0;
    int nr = 0;
    int hide = 0;
    int min_char_pos = 0;
    ...
    if (CONFIG_NEW_SOCIALS == true)
    {
        ...
        *next_soc = '\0';
        while (!feof(fl))
        {
            trash = fgets(next_soc, MAX_STRING_LENGTH, fl);
            if (*next_soc == '~')
            {
                top_of_socialt++;
            }
